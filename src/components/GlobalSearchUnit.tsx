...
const GlobalSearchUnit = ({ selectFields, onSubmit, placeholder, isDependentSelect }: GlobalSearchUnitProps) => {
    const [form] = Form.useForm();
    const [trackFormFields, setTrackFormFields] = useState<Record<string, unknown>>()
    ...
    return (
        <Form form={form} className="flex flex-row max-w-full space-x-4 ">
            ...
            <SelectFields selectFields={selectFields} trackFormFields={trackFormFields} trackChange={updateTracker}/>
...
const SelectFields = ({ selectFields, trackFormFields, trackChange }: { selectFields?: SelectField[], trackFormFields?: Record<string, unknown>, trackChange: (isDelete: boolean) => void }) => {
    return (
        <>
                ...
                field.dependenceName ? (
                    <Form.Item key={field.name} name={field.name}>
                        ...
                        >
                            {field.options
                                .filter(option => {
                                if (!option.dependence || !field.dependenceName || !trackFormFields) {
                                    return true;
                                }
                                const dependentValue = trackFormFields[field.dependenceName];
                                return option.dependence === (dependentValue?.toString() ?? '');
                                })
                                .map(option => (
