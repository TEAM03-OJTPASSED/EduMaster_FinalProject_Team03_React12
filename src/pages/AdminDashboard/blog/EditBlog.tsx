...
const EditBlog: React.FC<EditBlogProps> = ({ initialValues, onSuccess }) => {
    ...
    useEffect(() => {
        if (initialValues) {
            ...
            if (initialValues.tags) {
                const uniqueOptions = Array.from(new Set(initialValues.tags)).map((tag) => ({ value: tag, label: tag }));
    ...
    return (
        <Form form={form} name="edit-blog" onFinish={onFinish} layout="vertical" style={{ display: "flex", height: "100%" }}>
            {/* Left Section */}
